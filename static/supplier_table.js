...
function AddItemToTable(name, age, mobile, bName, bAddress, bDescription) {
  ...
  td5.innerHTML = bName;
  td6.innerHTML = bAddress;
  td7.innerHTML = bDescription;
...
async function GetAllDataOnce(){
  const querySnapshot =await getDocs(collection(db, "suppInfo"));
  var supplier =[];

  querySnapshot.forEach(doc=>{
    supplier.push(doc.data());
  });
  AddAllItemToTable(supplier);
}
